# -*- coding: utf-8 -*-
"""
Created on Tue Jul 16 10:29:53 2024
...
#%% Default GBDT model
# Defining the GBDT model
gbdt_model = GradientBoostingRegressor(n_estimators=150, learning_rate=0.2, max_depth=4, random_state=42)
gbdt_model.fit(X_train, y_train.ravel())

# Prediction and evaluation of the default model
test_predictions_gbdt = gbdt_model.predict(X_test)
train_predictions_gbdt = gbdt_model.predict(X_train)

print("\n\n ================TEST SET==========================================")
# Metrics for the test set
r2_test = r2_score(y_test, test_predictions_gbdt)
mse_test = mean_squared_error(y_test, test_predictions_gbdt)
print(f"R^2 coefficient of determination (test): {r2_test}")
print(f"Mean squared error MSE (test): {mse_test}")

print("\n\n ================TRAIN SET==========================================")
# Metrics for the training set
r2_train = r2_score(y_train, train_predictions_gbdt)
mse_train = mean_squared_error(y_train, train_predictions_gbdt)
...
cv_scores = cross_val_score(gbdt_model, X_train, y_train.ravel(), cv=25, scoring='r2')
mean_r2 = cv_scores.mean()
print("Average R^2 after cross-validation:", mean_r2)

cv_scores_mse = cross_val_score(gbdt_model, X_train, y_train.ravel(), cv=25, scoring='neg_mean_squared_error')
mean_mse = cv_scores_mse.mean()
print("Average MSE after cross-validation:", -mean_mse)

#%% Plots for default model
# Generating plots
test_predictions_gbdt = pd.Series(test_predictions_gbdt)
pred_df = pd.DataFrame(y_test, columns=['Test TRUE Y'])
pred_df = pd.concat([pred_df, test_predictions_gbdt], axis=1)
pred_df.columns = ['Test true y', 'Pred']

train_predictions_gbdt = pd.Series(train_predictions_gbdt)
train_df = pd.DataFrame(y_train, columns=['Train TRUE Y'])
train_df = pd.concat([train_df, train_predictions_gbdt], axis=1)
...
sns.scatterplot(x='Test TRUE Y', y='Pred', data=test_pred_df_grid, alpha=0.6).set_title("GBDT Grid")
...
sns.scatterplot(x='Test TRUE Y', y='Pred', data=test_pred_df_bayes, alpha=0.6).set_title("GBDT Bayes")
...
train_predictions_grid.to_csv(os.path.join(directory_path, 'train_set_GBDT_U_GRID.csv'), sep=';', encoding='utf-8', index=False)
test_predictions_grid.to_csv(os.path.join(directory_path, 'test_set_GBDT_U_GRID.csv'), sep=';', encoding='utf-8', index=False)
train_predictions_bayes.to_csv(os.path.join(directory_path, 'train_set_GBDT_U_Bayes.csv'), sep=';', encoding='utf-8', index=False)
test_predictions_bayes.to_csv(os.path.join(directory_path, 'test_set_GBDT_U_Bayes.csv'), sep=';', encoding='utf-8', index=False)
...
dataG = dataG.T
dataB = dataB.T
file_pathG = os.path.join(directory_path, nazwa + '_U_DATA_GBDT_GRID.xlsx')
file_pathB = os.path.join(directory_path, nazwa + '_U_DATA_GBDT_BAYES.xlsx')
...
file_pathG_smooth = os.path.join(directory_path, nazwa + '_U_DATA_GBDT_GRID_smooth.xlsx')
file_pathB_smooth = os.path.join(directory_path, nazwa + '_U_DATA_GBDT_BAYES_smooth.xlsx')
...
#%%
# Save the smoothed data to new Excel files
file_pathG_smooth = os.path.join(directory_path, nazwa + '_U_DATA_GBDT_GRID_smooth_1.xlsx')
file_pathB_smooth = os.path.join(directory_path, nazwa + '_U_DATA_GBDT_BAYES_smooth_1.xlsx')
dataG_smooth.to_excel(file_pathG_smooth, index=False)
dataB_smooth.to_excel(file_pathB_smooth, index=False)
