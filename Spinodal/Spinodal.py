...
data_mat = scipy.io.loadmat('D4.mat')
...
#numL['pressure_scaled'] = numL.loc[indP, 'pressure'] * 10**-3
### Speed of sound fitting
from scipy.optimize import curve_fit
# Definicja funkcji prędkości dźwięku w funkcji ciśnienia
def cspin(p, *b):
    return (1 + b[0] * p) ** b[1]
def cs(T, data):
...
def plot_graph(Pf, rhoa, D4, TuD4, D4Hir, TuD4Hir, FS=12):
    ...
    for j in range(4):
        ...
        plt.errorbar(D4[ind[0], 1] * 1e-3, D4[ind[0], 2], yerr=12 * D4[ind[0], 2] / D4[ind[0], 2], fmt='x')
